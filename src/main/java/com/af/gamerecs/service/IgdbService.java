...
public class IgdbService {
    ...
    public List<IgdbGameDto> searchGames(String query) {
        String body = """
            fields id, name, cover.url, first_release_date, franchise.name, genres.name, game_modes.name, player_perspectives.name, platforms.name, keywords.name, themes.name, rating, rating_count;
            search "%s";
            limit 20;
        """.formatted(query);
